	...
	for (int row = 0; row < gridSize; ++row) {
		for (int column = 0; column < gridSize; ++column) {
			...
			if (row == gridSize - 1 && column == gridSize - 1) {
				...
				QString imagePath = QString(":%1/game_images/%2.jpg")
									.arg(selectedLevel)
									.arg(row * gridSize + column + 1);
...
void bva::Puzzle::onViewOriginalClicked()
{
	...
	QString originalPixmap = QString(":%1/game_images_origin/"
									 "game_images_origin.jpg")
									 .arg(selectedLevel);
...
void bva::Puzzle::checkWin()
{
	...
	for (int i = 0; i < gridSize * gridSize - 1; ++i) {
		QPixmap pixmap(QString(":%1/game_images/%2.jpg")
					   .arg(selectedLevel)
					   .arg(i + 1));
	...
	if (selectedLevel == 1) {
		...
		QPixmap winPixmap(QString(":%1/win_image/win_image.jpg")
						  .arg(selectedLevel));
...
void bva::Puzzle::shufflePuzzle(QList<QPushButton*>& cells,
